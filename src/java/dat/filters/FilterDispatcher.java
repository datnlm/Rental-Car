...
public class FilterDispatcher implements Filter {
    ...
    private final String HOME = "SearchController";
    ...
    public FilterDispatcher() {
        ...
        USER.add("UpdateController");
        USER.add("DeleteController");
        USER.add("SaveController");
        USER.add("ViewHistoryController");
        USER.add("viewHistory.jsp");
        USER.add("viewDetailHistory.jsp");
        USER.add("DeleteHistoryController");
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            if (uri.contains("login.jsp")
                    ...
                    || uri.contains("VerifyController")
                    || uri.contains(".css")
                    || uri.contains(".jpeg")
                    || uri.contains("verify.jsp")
                    || uri.contains("search.jsp")
                    || uri.contains("SearchController")
                    || uri.contains("ResendEmailController")) {
                ...
                if (session == null || session.getAttribute("LOGIN_USER") == null) {
                    if (resource.contains("AddController")
                            || resource.contains("viewCart")
                            || resource.contains("viewHistory")
                    ...
                    String roleID = user.getStatus();
                    if (roleID.equals("Active") && resource.equals("viewCart")) {
                        req.getRequestDispatcher(VIEW_CART).forward(request, response);
                    } else if (roleID.equals("Active") && USER.contains(resource)) {
                        chain.doFilter(request, response);
                    } else {
                        res.sendRedirect(LOGIN);
                    }
